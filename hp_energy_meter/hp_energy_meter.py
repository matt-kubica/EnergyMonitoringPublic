...
mqttClientName                                = 'hp_energy_meter'
mqttBrokerHost                                = '0.0.0.0'
mqttSubscribedTopic                           = 'home/energy_monitoring/hp_energy_meter'
...
registersTableName                            = 'le03mwct_registers'


logFilePath                                   = '/home/pi/logs/hp_energy_meter.log'
logger                                        = None
...
class EnergyMeter(minimalmodbus.Instrument):
    ...
    def __init__(self, portname, slaveaddress = 1, baudrate = 9600, parity = serial.PARITY_EVEN, timeout = 0.5):
    ...
    def getRegisterData(self, address, datatype, functioncode = 3):
        if(datatype == 'float'):
            ...
            return self.read_long(address, functioncode = functioncode)
        else:
            raise TypeError
...
def updateInflux(portname, registersTable):
    ...
    try:
        energyMeter = EnergyMeter(portname, slaveaddress = int(slaveaddress))
    except:
...
def main():
    ...
    mainEnergyMeter = MainEnergyMeter()
    while(True):
        time.sleep(1)
    mainEnergyMeter.stop()
